...
_SELECT_VALUE_MAP: dict[str, str] = {
    ...
    # Boolean yes/no → Rieltor radio labels "Є"/"Немає"
    "так": "Є",
    "ні": "Немає",
    "yes": "Є",
    "no": "Немає",
...
class DictOfferFormFiller(
    ...
    def _fill_address_from_dict(self, root: Locator, address_data: dict) -> None:
        ...
        # Комерційна та Паркомісце: поле «Кадастровий номер» необов'язкове й лише
        # заважає (зайва геоприв'язка/валідація) — не заповнюємо його навіть тоді,
        # коли номер прийшов з CRM. Кадастр лишаємо тільки для Будинок/Ділянка.
        _pt = str(self.property_type).lower()
        if cadastral and (_pt == "комерційна" or _pt.startswith("паркомісце")):
            logger.info("Кадастровий номер пропущено для %s (поле необов'язкове)", self.property_type)
            cadastral = None
        ...
        if condo:
            ...
            if street:
                ...
                if street_ctrl and not self._control_has_value(street_ctrl):
                    logger.warning(
                        "Вулиця '%s' зникла після заповнення Будинку — повторне заповнення",
                        street,
                    )
                    self._fill_autocomplete(sec, "Вулиця", street)
