...
if __name__ == '__main__':
    ...
    new_offer = Offer(
        ...
        interior=PhotoBlock(
            description="Повністю укомплектована меблями та всією необхідною технікою для життя без зайвих турбот: вбудований холодильник, індукційна плита, духова шафа, мікрохвильова піч, посудомийна машина, пральна та сушильна машини, телевізор, витяжка, бойлер. Додатково встановлені система очищення води та центральне кондиціонування, що забезпечує комфорт у будь-яку пору року.",
        ),
        yard=PhotoBlock(
            description="ЖК Creator City є концепція «місто в місті» - вся необхідна для життя інфраструктура знаходиться на території комплексу. Для безпеки майбутніх мешканців в громадських місцях встановлять камери відеоспостереження, внутрішні двори огородять парканом, а увійти в під'їзд та ліфт можна буде тільки з картою-пропуском. Для дітей різного віку розмістять кілька ігрових комплексів, для спортсменів — вуличні тренажери і футбольне поле, а родзинкою комплексу стане власний ландшафтний парк площею 2 га з водоймою. Щоб комфортному відпочинку не заважали автомобілі, забудовник передбачив підземний дворівневий паркінг з ліфтом.",
        ),
        infrastructure=PhotoBlock(
            description="Локація - ще одна сильна сторона. Поруч зелений парк імені Івана Багряного, Київський зоопарк, метро Лук’янівська та Шулявська, КПІ, інноваційний простір Unit City, житлові комплекси Crystal Park Tower та інші знакові об’єкти району. Тут зручно жити, працювати й відпочивати.",
        ),
    )
