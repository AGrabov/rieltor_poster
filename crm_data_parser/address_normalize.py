...
# Мінімальна схожість folded-токена для впевненого збігу назви.
_VALUE_MATCH_THRESHOLD = 0.74


def _value_tokens(text: str) -> list[str]:
    """Folded значущі токени (без слів-типів вулиці) для звірки незалежно від порядку."""
    out: list[str] = []
...
def address_value_matches(desired: str, current: str, *, threshold: float = _VALUE_MATCH_THRESHOLD) -> bool:
    """Чи прийнятне значення ``current`` у адресному полі для бажаного ``desired``.

    Призначене для автокомпліту: не приймати чуже значення лише тому, що список
    закрився (раніше "Поділ" мовчки приймався як "Печерський", а
    "Велика Васильківська" — як "Велика Кільцева").

    Порівнює значущі folded-токени (без слів-типів вулиці) НЕЗАЛЕЖНО ВІД ПОРЯДКУ:
    кожен токен ``desired`` має фуззі-збігтися (SequenceMatcher >= threshold) з
    якимось токеном ``current``. Це приймає легітимні відмінності написання
    ("Болсунівська" → "Болсуновська вул.", "Шота Руставелі" → "Руставелі Шота"),
    але відхиляє іншу вулицю/район.
    """
    q_tokens = _value_tokens(desired)
