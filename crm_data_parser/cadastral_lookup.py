...
# Типи, для яких шукаємо/дозаповнюємо кадастровий номер. Комерційна та
# Паркомісце виключені свідомо: поле необов'язкове на rieltor.ua й лише заважає.
...
def enrich_offer_data_with_cadastral(offer_data: dict) -> bool:
    ...
    Перевіряє тип об'єкта через CRM_TYPE_TO_SCHEMA і обробляє лише типи, для
    яких кадастр потрібен (Будинок/Ділянка). Коли є
    ...
    if not cadnum:
        # Слід «шукали, але не знайшли»: без нього «порожнє поле» не відрізнити
        # від «пошук не запускався». Поле обов'язкове для Будинок/Ділянка —
        # без номера об'єкт впаде на валідації, тож попереджаємо явно.
        logger.warning(
            "Кадастр не знайдено за адресою для %s (%s, %s, буд. %s) — поле лишиться порожнім",
            label,
            address.get("Місто") or "?",
...
def fill_missing_cadastral_numbers(max_count: int | None = None) -> int:
    ...
    Запитує БД, фільтрує за типами (Будинок, Ділянка),
