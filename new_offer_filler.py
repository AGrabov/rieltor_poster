...
class NewOfferFormFiller:
    ...
    def _fill_address(self, root: Locator, offer: Offer) -> None:
        ...
        if a.condo_complex and a.condo_complex.startswith("ЖК "):
            a.condo_complex = a.condo_complex.replace("ЖК ", "")
    ...
    def _control_has_value(self, ctrl: Locator) -> bool:
        """Detect whether a MUI control is already filled.


        MUI Autocomplete may show selected value as chips/text while input.value is empty.
        """
        try:
        ...
        # chips
        try:
            if ctrl.locator(".MuiChip-root, .MuiChip-label").count():
                return True
        except Exception:
            pass


        # rendered text inside input root
        try:
            txt = " ".join(t.strip() for t in ctrl.all_inner_texts() if t.strip())
            if txt:
                return True
        except Exception:
            pass
    ...
    def _fill_autocomplete(self, section: Locator, key: str, value: str, *, next_key: str | None = None) -> None:
        ...
        # SKIP: уже заполнено (учитываем chips/рендер)
        if self._control_has_value(ctrl):
            try:
                cur = (inp.input_value() or "").strip()
            except Exception:
                cur = ""
        ...
        def _try_pick() -> bool:
            ...
            is_house = key == "house_number"
            return self._pick_autocomplete_option_and_verify(
                inp, desired,
                section=section, next_key=next_key,
                allow_single_option=is_house,
                allow_free_text=is_house,
            )
